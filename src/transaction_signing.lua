...
function run(input)
    ...
    local asn1ECKey = createASN1privateKey(string.sub(childKey.key, 3, 66))
    local blob = Blob.from_hex(asn1ECKey)
    local ecChildKey = importECKey(blob)
    local signature = assert(ecChildKey:sign { data = Blob.from_bytes(input.message), hash_alg = "SHA512" }).signature
